  ...
  return (
    <div className="bg-nutri-warm-bg">
      ...
      <div className="text-center pt-16 pb-12 px-8">
        ...
        <p className="text-nutri-green-forest font-bold text-base mt-3">
          Last updated: March 29, 2026
        </p>
      </div>

      {/* Categories */}
      <div className="max-w-[800px] mx-auto px-8 pb-12 flex flex-col gap-10">
        {categories.map((cat, idx) => (
          <div
            key={cat.title}
            className="bg-white rounded-3xl border border-nutri-green-border p-10 shadow-sm"
          >
            {/* Header */}
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-extrabold text-nutri-dark-text text-2xl">
                {cat.title}
              </h2>
              {cat.required ? (
                <span className="bg-nutri-green-pale text-nutri-green-dark font-bold text-xs px-3 py-1 rounded-full">
                  {cat.badge}
                </span>
              ) : (
                <Switch
            ...
            {/* Description */}
            <p className="text-nutri-dark-body text-base leading-relaxed mb-8">
              {cat.description}
            </p>

            {/* Cookie Table */}
            <div className="bg-nutri-warm-bg rounded-2xl overflow-hidden">
              <div className="grid grid-cols-3 px-6 py-3 border-b border-nutri-green-border/50">
                <span className="font-bold text-nutri-green-dark text-xs uppercase tracking-wider">
                  Cookie
                </span>
                <span className="font-bold text-nutri-green-dark text-xs uppercase tracking-wider">
                  Duration
              ...
              {cat.cookies.map((cookie) => (
                <div
                  key={cookie.name}
                  className="grid grid-cols-3 px-6 py-3 border-b border-nutri-green-border/30 last:border-0"
                >
                  <span className="text-nutri-dark-text font-bold text-sm">
                    {cookie.name}
                  </span>
                  <span className="text-nutri-dark-body text-sm">
                    {cookie.duration}
                  </span>
                  <span className="text-nutri-dark-body text-sm">
                    {cookie.purpose}
      ...
      {/* Sticky Save Bar */}
      <div className="bg-white border-t border-nutri-green-border py-8">
        <div className="max-w-[800px] mx-auto px-8">
          <div className="bg-nutri-dark-text rounded-3xl p-8 flex items-center justify-between">
            <div>
            ...
            <div className="flex gap-4">
              <button
                ...
                className={cn(
                  "rounded-full px-6 py-3 font-bold text-sm",
